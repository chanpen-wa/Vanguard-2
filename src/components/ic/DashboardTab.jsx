...
import {
  CalendarDays, Download, X, Activity, Hospital, Bug,
  Search, Eye, ChevronDown, Printer,
} from "lucide-react";
import {
...
// ==========================================
const COLORS = {
  HAI: "#e11d48", POA: "#f97316", Pending: "#f59e0b", Discarded: "#94a3b8",
};

const CATEGORY_COLORS = {
  BSI: "#3b82f6", UTI: "#8b5cf6", PNEU: "#06b6d4", SSI: "#f43f5e",
  GI: "#eab308", CVS: "#ec4899", EENT: "#14b8a6", CNS: "#6366f1",
  OST: "#84cc16", REPR: "#a855f7", LRI: "#06b6d4", OTHER: "#64748b",
};
...
export default function DashboardTab({
  ...
  const stats = useMemo(() => {
    ...
    const result = { total, pending, hai, poa, discarded, reviewed, haiRate, avgHospitalDay };
  ...
  const trendData = useMemo(() => {
    ...
    filtered.forEach((a) => {
      const key = new Date(a.created_at).toLocaleDateString("th-TH", { day: "numeric", month: "short" });
      if (dates[key]) {
        dates[key].Total += 1;
  ...
  const wardData = useMemo(() => {
    const map = {};
    filtered.forEach((a) => {
      const ward = a.ward_name || a.detailed_analysis_json?.ward_name || "ไม่ระบุ";
      if (!map[ward]) map[ward] = { name: ward, HAI: 0, POA: 0, Pending: 0, Total: 0 };
      map[ward].Total += 1;
    ...
    return Object.values(map)
      .map((w) => ({ ...w, haiRate: w.Total > 0 ? ((w.HAI / (w.HAI + w.POA || 1)) * 100).toFixed(1) : 0 }))
  ...
  const categoryData = useMemo(() => {
    ...
    filtered.forEach((a) => {
      ...
      if (!map[cat]) map[cat] = { name: cat, count: 0, HAI: 0, POA: 0, color: CATEGORY_COLORS[cat] || COLORS.OTHER };
      map[cat].count += 1;
  ...
  const monthlyData = useMemo(() => {
    ...
    filtered.forEach((a) => {
      ...
      const key = d.toLocaleDateString("th-TH", { month: "short", year: "numeric" });
  ...
  const caseTableData = useMemo(() => {
    let data = filtered;
    if (caseStatusFilter !== "all") data = data.filter((a) => a.status === caseStatusFilter);
    return data.sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
  }, [filtered, caseStatusFilter]);

  useEffect(() => { setCurrentPage(1); }, [caseStatusFilter, caseSearch]);

  const totalPages = Math.max(1, Math.ceil(caseTableData.length / ITEMS_PER_PAGE));
  const paginatedCases = caseTableData.slice((currentPage - 1) * ITEMS_PER_PAGE, currentPage * ITEMS_PER_PAGE);
  ...
  const handleExportExcel = () => {
    ...
    const wardRows = [["หอผู้ป่วย", "ทั้งหมด", "HAI", "POA", "Pending", "HAI Rate"]];
    wardData.forEach((w) => wardRows.push([w.name, w.Total, w.HAI, w.POA, w.Pending, `${w.haiRate}%`]));
    const ws2 = XLSX.utils.aoa_to_sheet(wardRows);
    ws2["!cols"] = [{ wch: 30 }, { wch: 12 }, { wch: 10 }, { wch: 10 }, { wch: 10 }, { wch: 12 }];
    ...
    trendData.forEach((t) => trendRows.push([t.date, t.HAI, t.POA, t.Pending, t.Total]));
    const ws4 = XLSX.utils.aoa_to_sheet(trendRows);
    ws4["!cols"] = [{ wch: 15 }, { wch: 10 }, { wch: 10 }, { wch: 10 }, { wch: 10 }];
    XLSX.utils.book_append_sheet(wb, ws4, "📈 Trends");

    const monthlyRows = [["เดือน", "HAI", "POA", "รวม"]];
    monthlyData.forEach((m) => monthlyRows.push([m.month, m.HAI, m.POA, m.Total]));
    ...
    XLSX.writeFile(wb, `VanguardIC_Report_${new Date().toISOString().split("T")[0]}.xlsx`);
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 px-4 py-2 rounded-xl">
            ...
            <input
              ...
              onChange={(e) => setDashDates({ ...dashDates, start: e.target.value })}
            ...
            <input
              type="date"
              value={dashDates.end}
              onChange={(e) => setDashDates({ ...dashDates, end: e.target.value })}
              className="bg-transparent text-sm font-bold text-slate-700 outline-none"
            />
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
        <StatCard label="Total Cases" value={stats.total} color="bg-slate-100 text-slate-800" />
        <StatCard label="Pending" value={stats.pending} color="bg-amber-50 text-amber-600" />
        <StatCard label="HAI" value={stats.hai} color="bg-rose-50 text-rose-600" />
        <StatCard label="POA" value={stats.poa} color="bg-orange-50 text-orange-600" />
        <StatCard label="Discarded" value={stats.discarded} color="bg-slate-50 text-slate-500" />
        <StatCard label="HAI Rate" value={`${stats.haiRate}%`} color="bg-indigo-50 text-indigo-600" />
        <StatCard label="Avg Day" value={stats.avgHospitalDay} color="bg-purple-50 text-purple-600" />
      </div>

      {/* Charts */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 p-6 rounded-3xl border border-slate-200 shadow-sm bg-white">
          <h3 className="font-bold text-slate-800 mb-4">📈 Infection Trends ({trendRange} Days)</h3>
          <ResponsiveContainer width="100%" height={350}>
            <AreaChart data={trendData}>
              ...
              <Area type="monotone" dataKey="HAI" stroke={COLORS.HAI} strokeWidth={2} fill="url(#haiGrad)" dot={{ r: 3 }} />
              <Area type="monotone" dataKey="POA" stroke={COLORS.POA} strokeWidth={2} fill="url(#poaGrad)" dot={{ r: 3 }} />
              <Line type="monotone" dataKey="Pending" stroke={COLORS.Pending} strokeWidth={1} strokeDasharray="5 5" dot={false} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
        <div className="p-6 rounded-3xl border border-slate-200 shadow-sm bg-white">
          ...
          {pieData.length > 0 ? (
            <ResponsiveContainer width="100%" height={350}>
              <PieChart>
                <Pie data={pieData} innerRadius={70} outerRadius={110} paddingAngle={5} dataKey="value" stroke="none">
            ...
            <div className="h-[350px] flex items-center justify-center text-slate-400 font-medium">No data</div>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="p-6 rounded-3xl border border-slate-200 shadow-sm bg-white">
          <h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2"><Hospital className="w-5 h-5 text-indigo-500" /> Ward Performance</h3>
          <ResponsiveContainer width="100%" height={350}>
            <BarChart data={wardData.slice(0, 10)} layout="vertical">
              ...
              <Legend iconType="circle" />
              <Bar dataKey="HAI" fill={COLORS.HAI} stackId="a" barSize={20} name="HAI" />
              <Bar dataKey="POA" fill={COLORS.POA} stackId="a" barSize={20} name="POA" />
              <Bar dataKey="Pending" fill={COLORS.Pending} stackId="a" barSize={20} name="Pending" />
            </BarChart>
          </ResponsiveContainer>
        </div>
        <div className="p-6 rounded-3xl border border-slate-200 shadow-sm bg-white">
          <h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2"><Bug className="w-5 h-5 text-indigo-500" /> Infection Categories</h3>
          <ResponsiveContainer width="100%" height={350}>
            <BarChart data={categoryData}>
              ...
              <Tooltip contentStyle={{ borderRadius: "12px", border: "1px solid #e2e8f0" }} />
              <Legend iconType="circle" />
              <Bar dataKey="HAI" fill={COLORS.HAI} radius={[8, 8, 0, 0]} name="HAI" />
              <Bar dataKey="POA" fill={COLORS.POA} radius={[8, 8, 0, 0]} name="POA" />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="rounded-3xl border border-slate-200 shadow-sm bg-white overflow-hidden">
        ...
        >
          <h3 className="font-bold text-slate-800 flex items-center gap-2"><Eye className="w-5 h-5 text-indigo-500" /> Case Details ({caseTableData.length.toLocaleString()})</h3>
          <div className={`transition-transform ${showCaseTable ? "rotate-180" : ""}`}><ChevronDown className="w-5 h-5 text-slate-400" /></div>
        </button>
        {showCaseTable && (
          <div className="px-6 pb-6">
            <div className="flex flex-wrap gap-3 mb-4">
              <div className="relative flex-1 min-w-[200px]">
                ...
                <input type="text" placeholder="🔍 ค้นหา HN หรือชื่อผู้ป่วย..." value={caseSearch} onChange={(e) => setCaseSearch(e.target.value)} className="w-full pl-10 pr-4 py-2.5 border border-slate-200 rounded-xl text-sm outline-none focus:border-indigo-400 bg-slate-50 focus:bg-white" />
                {isSearching && <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-slate-400">⏳</span>}
              </div>
              <select value={caseStatusFilter} onChange={(e) => setCaseStatusFilter(e.target.value)} className="px-4 py-2.5 border border-slate-200 rounded-xl text-sm font-bold outline-none bg-slate-50">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-slate-50 text-left">
                    {["วันที่", "HN", "ชื่อผู้ป่วย", "หอผู้ป่วย", "หมวดหมู่", "ผล AI", "สถานะ"].map((h) => (
                      <th key={h} className="p-3 font-bold text-slate-500 text-xs uppercase tracking-wider whitespace-nowrap">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {paginatedCases.map((a) => (
                    <tr key={a.id} className="border-b border-slate-50 hover:bg-indigo-50/30 transition-colors">
                      <td className="p-3 text-xs whitespace-nowrap">{new Date(a.created_at).toLocaleDateString("th-TH")}</td>
                      <td className="p-3 font-mono text-xs">{a.hn}</td>
                      <td className="p-3 font-medium text-sm">{a.patient_name}</td>
                      <td className="p-3 text-xs whitespace-nowrap">{a.ward_name || a.detailed_analysis_json?.ward_name || "-"}</td>
                      <td className="p-3"><span className="px-2 py-0.5 rounded-full text-[10px] font-bold whitespace-nowrap" style={{ backgroundColor: (CATEGORY_COLORS[a.device_type] || "#64748b") + "20", color: CATEGORY_COLORS[a.device_type] || "#64748b" }}>{a.device_type}</span></td>
                      <td className="p-3 text-xs max-w-[200px] truncate">{a.auto_assess_result}</td>
                      <td className="p-3"><span className={`px-2 py-0.5 rounded-full text-[10px] font-bold whitespace-nowrap ${a.status === "Confirmed" ? "bg-rose-100 text-rose-700" : a.status === "POA" ? "bg-orange-100 text-orange-700" : a.status === "Pending" ? "bg-amber-100 text-amber-700" : "bg-slate-100 text-slate-500"}`}>{a.status === "Confirmed" ? "HAI" : a.status}</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
              {caseTableData.length === 0 && <p className="text-center text-slate-400 py-12">ไม่พบข้อมูล</p>}
            </div>
            {totalPages > 1 && (
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mt-4 pt-4 border-t border-slate-100">
                <p className="text-xs text-slate-500">แสดง {(currentPage - 1) * ITEMS_PER_PAGE + 1} - {Math.min(currentPage * ITEMS_PER_PAGE, caseTableData.length)} จาก {caseTableData.length.toLocaleString()} รายการ</p>
                <div className="flex items-center gap-1">
                  <button onClick={() => setCurrentPage(1)} disabled={currentPage === 1} className="w-8 h-8 rounded-lg text-xs font-bold disabled:opacity-30 hover:bg-slate-100">««</button>
                  <button onClick={() => setCurrentPage((p) => Math.max(1, p - 1))} disabled={currentPage === 1} className="w-8 h-8 rounded-lg text-xs font-bold disabled:opacity-30 hover:bg-slate-100">«</button>
                  {getPageNumbers().map((p, i) => p === "..." ? <span key={`dots-${i}`} className="w-8 h-8 flex items-center justify-center text-xs text-slate-300">...</span> : <button key={p} onClick={() => setCurrentPage(p)} className={`w-8 h-8 rounded-lg text-xs font-bold transition-all ${p === currentPage ? "bg-indigo-600 text-white shadow-sm" : "hover:bg-slate-100 text-slate-600"}`}>{p}</button>)}
                  <button onClick={() => setCurrentPage((p) => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className="w-8 h-8 rounded-lg text-xs font-bold disabled:opacity-30 hover:bg-slate-100">»</button>
                  <button onClick={() => setCurrentPage(totalPages)} disabled={currentPage === totalPages} className="w-8 h-8 rounded-lg text-xs font-bold disabled:opacity-30 hover:bg-slate-100">»»</button>
                </div>
              </div>
            )}
          </div>
...
function StatCard({ label, value, color }) {
  return (
    <div className={`p-4 rounded-2xl border border-slate-200 shadow-sm ${color.split(" ")[0]}`}>
